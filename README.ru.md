...
Ошибки, вывод `help` и приглашение REPL доступны на русском и английском:
...
По умолчанию — русский. Приоритет: флаг `--lang` > переменная `CALC_LANG` > русский.
...
окон», переключатель языка **RU/EN** (кнопка справа). Смена языка сразу
перерисовывает строки-ошибки; выбор сохраняется между запусками. Есть подсветка
синтаксиса и автосохранение текста.
...
v0.2.5. Весь workspace покрыт тестами (135 тестов: core + notepad), clippy `-D warnings`
чист. Готовы оба фронтенда — CLI и GUI-блокнот, с переключателем языка RU/EN. Сборки
exe + Linux публикуются по тегу в релизы.
